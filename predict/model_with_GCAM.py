...
class LocalAttentionModule(Model):
    ...
    def call(self, skip, main):
        # Generate attention weights from skip connection
        ...
        # Combine the attended skip connection with the main path
...
class BAMModule(Model):
    def __init__(self, filters):
        ...
        # Reshape for broadcasting the attention weights
        self.reshape = Reshape((1, 1, filters))
    ...
    def call(self, inputs):
        ...
        y = Multiply()([inputs, y])  # Element-wise multiplication for attention
        return y
    
# base model with GRU after layer_six
class base_model(Model):
    def __init__(self):
        super(base_model, self).__init__()
        # Initial layers and downsample layers
        ...
        # Local attention modules for skip connections
        ...
        # GRU layer after layer_six (each time step outputs 32-dimensional hidden state)
        self.reshape_to_3D = Reshape((-1, 32))  # Reshape (batch, height, width, channels) to (batch, timesteps, features)
        self.gru = GRU(32, return_sequences=True)  # GRU layer, each time step outputs a 32-dimensional hidden state
        self.reshape_back_to_4D = Reshape((-1, 1, 32))  # Reshape back to 4D (batch, height, width, channels)
        # BAM module after downsampling
        ...
        # Output layer
        self.out_layer = tf.keras.layers.Conv2D(filters=2, kernel_size=(9, 1), strides=(1, 1), padding='same')
    ...
    def call(self, x):
        ...
        # GRU after layer_six
        y6_reshaped = self.reshape_to_3D(y6)  # Reshape to 3D (batch, height, width, channels) -> (batch, timesteps, features)
        y_gru = self.gru(y6_reshaped)  # Pass through GRU (each timestep outputs 32-dimensional hidden state)
        y_gru_reshaped = self.reshape_back_to_4D(y_gru)  # Reshape back to 4D (batch, height, width, channels)
